...
def log_info(msg):
    print(
        f'INFO [{str(datetime.astimezone(datetime.now()).strftime("%d-%b-%Y_%H:%M:%S_%Z")).lstrip().rstrip()}]: {msg}')


def log_error(msg):
    print(
        f'ERROR [{str(datetime.astimezone(datetime.now()).strftime("%d-%b-%Y_%H:%M:%S_%Z")).lstrip().rstrip()}]: {msg}')
...
def read_protocol_file(filepath):
    ...
    df_testcases = pd.read_excel(filepath, engine='openpyxl',
                                 sheet_name='protocoltestcasedetails')  # ,keep_default_na=False
...
def check_combination_allowed(combinations_path, dict_combination):
    ...
    for index, row in pdf_schemacompare.iterrows():
        if (row['sourceconnectiontype'].strip().lower() == sourceconnectiontype and row[
            'targetconnectiontype'].strip().lower() == targetconnectiontype and row[
            'sourceformat'].strip().lower() == sourceformat and row['targetformat'].strip().lower() == targetformat):
            flag_allowed = 'Y'
            break
        else:
            flag_allowed = 'N'
...
def preproc_unnestfields(microBatchDF):
    ...
    while len(complex_fields) != 0:
        ...
        if (type(complex_fields[col_name]) == StructType):
            expanded = [col(col_name + '.' + k).alias((col_name + '_' + k).lower()) for k in
