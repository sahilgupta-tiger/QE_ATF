...
class generatePDF:
  ...
  def __init__(self):
    ...
    self.pdf.add_page()
    self.pdf.add_font('DejaVu', '', '/Workspace/Shared/QE_ATF_Latest/datf_core/dejavu/DejaVuSansCondensed.ttf', uni=True)
        ...
        for j,val in enumerate(row):
            ...
            if(col_factor == 1): 
              ...
              if(table_header[k] == 'Key Columns' or table_header[k] == 'Testcase Name'):
                ...
                self.pdf.multi_cell(col_width, cth, txt=str(val), border = 0, align= 'L')
              else:
                self.pdf.rect(x_pos,y_pos,col_width,factor*mth)
