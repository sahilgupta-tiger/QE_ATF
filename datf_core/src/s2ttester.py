...
class S2TTester:
    ...
    def execute_protocol(self, dict_protocol, df_testcases, output_path, combined_testcase_output_path, testcasetype,testcasesrunlist):
        ...
        if testcasetype == "count":
            ...
            df_protocol_summary = pd.DataFrame(columns=['Testcase Name', 'No of column has null in source','No of column has null in target','No of columns has null count match between source and target','No of Columns has null count mismatch between source and target', 'Test Result', 'Reason', 'Runtime'])
        ...
        for index, row in df_testcases.iterrows():
            ...
            try:
                ...
                if testcasetype == "count":
                    ...
                    df_protocol_summary.loc[index] = [test_case_name,str(dict_testresults['No of column has null in source']),str(dict_testresults['No of column has null in target']),str(dict_testresults['No of columns has null count match between source and target']),str(dict_testresults['No of Columns has null count mismatch between source and target']),dict_compareoutput['test_result'], dict_compareoutput['result_desc'], str(testcase_exectime)]
    ...
    def compare_data(self, compare_input, testcasetype):
        ...
        if (testcasetype == 'null'):
            ...
            dict_results = {
                'Test Result': test_result, 'No of column has null in source':f"{totalsrcnullcols:,}",'No of column has null in target':f"{totaltgtnullcols:,}", 'No of columns has null count match between source and target':f"{columns_match_null_count:,}", 'No of Columns has null count mismatch between source and target': f"{columns_mis_null_count:,}"}
